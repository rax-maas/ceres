...
public class DownsampleJobProcessorTest {
  ...
  @Test
  public void testDownsampleJobProcessorMethods() throws InterruptedException {
    Instant now = Instant.now();
    when(downsampleTrackingService.claimJob(any(), anyString())).thenReturn(Flux.just("free"));
    when(downsampleTrackingService.getTimeSlot(any(), anyString()))
        .thenReturn(Mono.just(String.valueOf(now.getEpochSecond())));

    verify(downsampleTrackingService, times(1)).freeJob(0, "PT15M");
